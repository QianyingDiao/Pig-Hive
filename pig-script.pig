...
grunt> outputs = FOREACH filtered GENERATE group AS bigram, SUM(i_bigrams.occurrences) AS total_occ, SUM(i_bigrams.books) AS total_books, ROUND(SUM(i_bigrams.occurrences)/SUM(i_bigrams.books))/100.0 AS average, MIN(i_bigrams.year) AS first_year, MAX(i_bigrams.year) AS last_year, COUNT(i_bigrams.year) AS number_year;
